...
export function OrderPayForm({
  ...
  const chargeCents =
    policy.payInFullOnly || paymentChoice === "full" ? policy.fullCents : policy.depositCents;
  ...
  return (
    <div className="card mt-6">
      ...
      {policy.payInFullOnly ? (
        <p className="mt-6 text-sm text-[var(--warm-gray)]">{policy.reason}</p>
      ) : (
      ...
      {stripeEnabled ? (
        <button type="button" onClick={handlePay} disabled={loading} className="btn-primary mt-6 w-full">
          {loading
            ? "Redirecting..."
            : policy.payInFullOnly || paymentChoice === "full"
